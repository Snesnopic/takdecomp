...
def run() -> int:
    ...
    for sr, ch, bits, sig, preset in combos:
        ...
        wav_path = f"{base}.wav"
        tak_path = f"{base}_{preset}_rt.tak"
        dec_path = f"{base}_{preset}_rt_dec.wav"
        ...
        dec_ok, dec_err = run_cmd(
            [TAKC_BIN, "-d", "-overwrite", "-q", tak_path, dec_path]
        )
        if not dec_ok:
        ...
        # 3. Compare PCM — must be lossless
