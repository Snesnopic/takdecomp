...
void Decoder::decorrelate(int c1, int c2, int length, BitStreamReader& gb) {
    ...
    case 5: /* side/right with scale factor */
        dshift = gb.get_bits1() ? gb.get_bits(4) + 1 : 0;
    ...
    case 6:
        ...
        [[fallthrough]];
    case 7: {
        if (length < 256) throw std::runtime_error("Invalid decorrelate length");

        dshift = gb.get_bits1() ? gb.get_bits(4) + 1 : 0;
        int filter_order = 8 << gb.get_bits1();
        int dval1 = gb.get_bits1();
        int dval2 = gb.get_bits1();
        ...
        p1 += order_half;
        int x = residues_.size() - filter_order;
        int tmp;
