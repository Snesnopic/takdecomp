...
namespace takdecomp {
    ...
    int Decoder::get_nb_samples(const int sample_rate, FrameSizeType type) {
        ...
        if (type_val <= static_cast<uint8_t>(FrameSizeType::Fs250ms)) {
            ...
            max_nb_samples = (sample_rate * frame_duration_type_quants[static_cast<uint8_t>(FrameSizeType::Fs250ms)]) >>
                             constants::FRAME_DURATION_QUANT_SHIFT;
        } else {
        ...
        if (nb_samples_ < 16) {
            ...
            if (info.codec == CodecType::MonoStereo) {
                ...
                if (gb.get_bits1() != 0u) {
                    ...
                    if (chan > channels_) {
                        throw std::runtime_error("Invalid multichannel chan");
                    }

                    for (int i = 0; i < chan; i++) {
                        int const nbit = gb.get_bits(4);
                        if (nbit >= channels_ || ((ch_mask & (1 << nbit)) != 0)) {
                            throw std::runtime_error("Invalid channel mask");
                        }
                        ...
                        if (mcdparams_[i].present != 0) {
                            ...
                            if (mcdparams_[i].chan2 >= channels_) {
                                throw std::runtime_error("Invalid chan2");
                            }
