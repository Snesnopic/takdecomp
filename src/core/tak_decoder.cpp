...
constexpr std::array<uint32_t, 256> crc24_table = {
    0x000000, 0x864DEF, 0x9048E1, 0x16050E, 0xBB4AFA, 0x3D0715, 0x2B021B, 0xAD4FF4, 
    ...
    if (gb.get_bits1()) {
        ...
        if (gb.get_bits1()) {
            for (int i = 0; i < s.channels; i++) {
                int value = gb.get_bits(constants::FORMAT_CH_LAYOUT_BITS);
                if (value < static_cast<int>(tak_channel_layouts.size())) {
...
void Decoder::decode_frame_header(BitStreamReader& gb, StreamInfo& ti) {
    ...
    if (ti.flags & constants::FRAME_FLAG_IS_LAST) {
    ...
    if (ti.flags & constants::FRAME_FLAG_HAS_INFO) {
        StreamInfo parsed_info = parse_streaminfo(gb);
    ...
    if (ti.flags & constants::FRAME_FLAG_HAS_METADATA) {
    ...
    size_t header_len = gb.get_position_bytes();
    
    // Read the expected CRC as a 24-bit big-endian integer from the aligned byte position
    const uint8_t* buf = gb.get_data().data();
    uint32_t expected_crc = (buf[header_len] << 16) | (buf[header_len + 1] << 8) | buf[header_len + 2];
    
    gb.skip_bits(24);
    
    uint32_t crc = 0xCE04B7;
    for (size_t i = 0; i < header_len; i++) {
        crc = crc24_table[(uint8_t)crc ^ buf[i]] ^ (crc >> 8);
    }
    crc &= 0xFFFFFF;
...
size_t Decoder::decode_frame(std::span<const uint8_t> data, StreamInfo& info, std::vector<std::vector<int32_t>>& output) {
    ...
    int64_t base_align = (((sample_rate_ + 511LL) >> 9) + 3) & ~3LL;
    uval_ = base_align << shift;
    subframe_scale_ = base_align << 1;

    nb_samples_ = info.last_frame_samples ? info.last_frame_samples : info.frame_samples;
    ...
    if (nb_samples_ < 16) {
        ...
        if (info.codec == CodecType::MonoStereo) {
            ...
            if (gb.get_bits1()) {
                int ch_mask = 0;
                int chan = gb.get_bits(4) + 1;
                if (chan > channels_) throw std::runtime_error("Invalid multichannel chan");

                for (int i = 0; i < chan; i++) {
                    int nbit = gb.get_bits(4);
                    if (nbit >= channels_ || (ch_mask & (1 << nbit))) throw std::runtime_error("Invalid channel mask");

                    mcdparams_[i].present = gb.get_bits1();
                    if (mcdparams_[i].present) {
                        ...
                        if (mcdparams_[i].index == 1) {
                            if (nbit == mcdparams_[i].chan2 || (ch_mask & (1 << mcdparams_[i].chan2))) {
                                throw std::runtime_error("Invalid multichannel config");
                            }
                            ch_mask |= 1 << mcdparams_[i].chan2;
                        } else if (!(ch_mask & (1 << mcdparams_[i].chan2))) {
                            throw std::runtime_error("Invalid multichannel config");
                        }
                    }
            ...
            for (int i = 0; i < channels_; i++) {
                if (mcdparams_[i].present && mcdparams_[i].index == 1) {
...
bool takdecomp::Decoder::check_crc24(std::span<const uint8_t> data) {
    ...
    for (size_t i = 0; i < len; i++) {
        crc = crc24_table[(uint8_t)crc ^ data[i]] ^ (crc >> 8);
    }
    crc &= 0xFFFFFF;
    
    uint32_t expected_crc = data[len] | (data[len + 1] << 8) | (data[len + 2] << 16);
