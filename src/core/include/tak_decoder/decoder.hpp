...
private:
    void decode_lpc(int32_t* coeffs, int mode, int length);
    ...
    int get_unary(BitStreamReader& gb, int step, int max);
    ...
    int get_nb_samples(int sample_rate, FrameSizeType type);
    bool check_crc24(std::span<const uint8_t> data);
